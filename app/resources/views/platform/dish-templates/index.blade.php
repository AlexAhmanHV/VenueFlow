<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-slate-900">Plattform · Rättkatalog</h2>
    </x-slot>
    ...
    <section class="vf-container space-y-5 py-8">
        ...
        <div class="vf-card p-6">
            <h3 class="text-lg font-semibold">Ny rättmall</h3>
            <form method="POST" action="{{ route('platform.dish-templates.store') }}" enctype="multipart/form-data" class="mt-4 grid gap-3 md:grid-cols-5">
                @csrf
                ...
                <input type="hidden" name="crop_x" id="dish-crop-x">
                <input type="hidden" name="crop_y" id="dish-crop-y">
                <input type="hidden" name="crop_w" id="dish-crop-w">
                <input type="hidden" name="crop_h" id="dish-crop-h">

                <div id="dish-dropzone" class="md:col-span-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
                    ...
                    <input type="file" id="dish-image-input" name="image" accept="image/*" class="mt-3 block w-full text-sm">
                    <img id="dish-image-preview" class="mt-3 hidden h-40 w-full rounded-lg object-cover" alt="Preview">
                </div>
                ...
                <button class="vf-btn-primary md:col-span-5">Skapa rättmall</button>
            </form>
        </div>
        ...
        <div class="vf-card overflow-hidden">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    <form method="POST" action="{{ route('platform.dish-templates.update', $template) }}" enctype="multipart/form-data" class="flex flex-wrap items-center gap-2">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <form method="POST" action="{{ route('platform.dish-templates.destroy', $template) }}">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-slate-500">Inga rättmallar ännu.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        (function () {
            const input = document.getElementById('dish-image-input');
            const preview = document.getElementById('dish-image-preview');
            const dropzone = document.getElementById('dish-dropzone');
            const xInput = document.getElementById('dish-crop-x');
            const yInput = document.getElementById('dish-crop-y');
            const wInput = document.getElementById('dish-crop-w');
            const hInput = document.getElementById('dish-crop-h');
